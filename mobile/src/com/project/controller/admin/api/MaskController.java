 ...
 * 技术QQ：2511251392
 */
@ControllerBind(controllerKey= "/admin/api/mask")
public class MaskController extends BaseController{
	...
	public void type() throws Exception{
		...
		List<Record> mask_list = Db.find("select m.*, t.title type_title from db_mask m left join db_type t on m.type_id=t.id where m.status=? and t.status=? and m.shop_id is null order by m.create_date desc", Mask.STATUS_ENABLE, Type.STATUS_ENABLE);
