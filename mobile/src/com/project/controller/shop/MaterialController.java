 ...
 * 技术QQ：2511251392
 */
@ControllerBind(controllerKey = "/shop/material", viewPath = "/shop/material")
public class MaterialController extends BaseController{
	...
	public void save() throws Exception{
		...
		for(String img_url : img_urls.split(",")){
			if(StrKit.notBlank(img_url)){
				Material material = Material.dao.findFirst("select * from db_material where shop_id=? and img_url=?", getLoginShopId(), img_url);
				if(material!=null){
	...
	public void edit() throws Exception{
		...
		Material material=Material.dao.findById(getPara("id"));
		if(!material.get("shop_id").toString().equals(getLoginShopId())){
			setAttr("msg", "没有操作权限");
			render("/shop/msg.htm");
			return;
		}
