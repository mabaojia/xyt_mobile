 ...
 * 技术QQ：2511251392
 */
@ControllerBind(controllerKey="/shop/mask/store", viewPath="/shop/mask/store")
public class MaskStoreController extends BaseController{
	/**
	 * 
	 * 
	 * 青岛小道福利信息技术服务有限公司
	...
	public void index() throws Exception{
		...
		String sWhere=" from db_mask m left join db_type t on m.type_id=t.id where m.status=? and t.status=? and m.shop_id is null";
		params.add(Mask.STATUS_ENABLE);
		...
		//蒙版分类
		List<Record> list=Type.getList(Type.TYPE_MASK, null);
		setAttr("list", list);
		render("list.htm");
	}
	/**
	...
	public void addAll() throws Exception{
		...
		for (String bid : getPara("bids").toString().split(",")) {
			if (StrKit.notBlank(bid)) {
				Mask mask = Mask.dao.findById(bid);
				img_list.add(mask.get("img_url").toString());
				img_urls+= mask.get("img_url").toString() + ",";
			}
		}
