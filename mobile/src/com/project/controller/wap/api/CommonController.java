 ...
 * 技术QQ：2511251392
 */
@ControllerBind(controllerKey = "/wap/api")
public class CommonController extends BaseController{
	...
	public void uploadImg() throws Exception{
	    ...
	    UserPicture user_picture=new UserPicture();
	    user_picture.set("user_id", getLoginUserId())
	    				.set("img_url", save_path + new_name)
	...
	public void search() throws Exception{
		...
		header();
		//贴图列表
		List<Record> material_list=Db.find("select m.*,t.title type_title from db_material m left join db_type t on m.type_id=t.id where m.status=? and m.title like ? and t.status=? and m.shop_id=? order by m.create_date desc", Material.STATUS_ENABLE, "%" + getPara("content") + "%", Type.STATUS_ENABLE, getLoginUser().get("shop_id"));
		setAttr("material_list", material_list);
		//图库列表
		List<Record> background_list=Db.find("select b.*,t.title type_title from db_background b left join db_type t on b.type_id=t.id where b.status=? and b.title like ? and t.status=? and b.shop_id=? order by b.create_date desc", Background.STATUS_ENABLE, "%" + getPara("content") + "%", Type.STATUS_ENABLE, getLoginUser().get("shop_id"));
